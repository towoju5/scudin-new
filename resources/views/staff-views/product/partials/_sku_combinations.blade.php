...
@foreach ($combinations as $key => $combination)
	...
	@if(strlen($str) > 0)
			<tr>
				...
				<td>
					<input type="number" name="price_{{ $str }}" value="{{ $unit_price }}" min="0" step="any" class="form-control" required>
				</td>
				<td>
					<input type="text" name="sku_{{ $str }}" value="{{ $sku }}" class="form-control" required>
				</td>
				<td>
					<input type="number" name="qty_{{ $str }}" value="1" min="0" max="100000" step="any" class="form-control" required>
				</td>
			</tr>
	@endif
@endforeach
