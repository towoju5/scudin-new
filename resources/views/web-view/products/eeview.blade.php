...
<div class="container">
  <div class="row">
    <div class="col-md-3">
      <a class="openbtn-tab" onclick="openNav()">
        <div style="font-size: 20px; font-weight: 600; " class="for-tab-display">☰ <?=  __('Open Sidebar') ?></div>
      </a>
    </div>
    <div class="col-md-9">
      <div class="row">
        <div class="col-md-6">
          ...
          {{-- <h1 class="h3 text-dark mb-0 headerTitle text-uppercase"><?=  __('product_by') ?> <?= $data['data_from'] ?> (<?= isset($brand_name) ? $brand_name : $data_from ?>)</h1> --}}
          <h1 class="h3 text-dark mb-3 headerTitle text-uppercase"><?= $data['data_from'] ?> <?=  __('products') ?> <?= isset($brand_name) ? '(' . $brand_name . ')' : '' ?></h1>
        </div>
        <div class="col-md-6 for-display">

          <button class="openbtn" onclick="openNav()">
            <div style="margin-bottom: -30%;">☰ <?=  __('Open Sidebar') ?></div>
          </button>
            ...
            <form class="" id="search-form" action="<?= route('products')  ?>" method="GET">
              ...
              <div class="form-inline flex-nowrap mr-3 mr-sm-4 pb-3 for-mobile">
                <label class=" opacity-75 text-nowrap mr-2 for-shoting" for="sorting">
                  <span class="mr-2"><?=  __('sort_by') ?></span></label>
                <select style="background: whitesmoke; appearance: auto;" class="form-control custom-select" onchange="filter(this.value)">
                  <option value="latest"><?=  __('Latest') ?></option>
                  <option value="low-high"><?=  __('low_high') ?> <?=  __('Price') ?> </option>
                  <option value="high-low"><?=  __('hight_low') ?> <?=  __('Price') ?></option>
    ...
    <aside class="col-lg-3 hidden-xs col-md-3 col-sm-4 SearchParameters" id="SearchParameters">
      ...
      <div class="cz-sidebar rounded-lg box-shadow-lg" id="shop-sidebar" style="margin-bottom: -10px;">
        <div class="cz-sidebar-header box-shadow-sm">
          <button class="close ml-auto" type="button" data-dismiss="sidebar" aria-label="Close"><span class="d-inline-block font-size-xs font-weight-normal align-middle"><?=  __('Dashboard') ?>Close sidebar</span><span class="d-inline-block align-middle ml-2" aria-hidden="true">&times;</span></button>
        </div>
              ...
              <div class="card">
                ...
                <div class="card-body ml-2" id="collapse-<?= $category['id'] ?>" style="display: none">
                  ...
                  <div class=" for-hover-lable card-header p-1">
                    <label style="cursor: pointer" onclick="location.href='<?= route('products', ['id' => $child['id'], 'data_from' => 'category', 'page' => 1]) ?>'"><?= $child['name'] ?></label>
                    <strong class="pull-right" style="cursor: pointer" onclick="$('#collapse-<?= $child['id'] ?>').toggle(400)">
                      <?= $child->childes->count() > 0 ? '+' : '' ?>
                    </strong>
                  </div>
                  <div class="card-body ml-2" id="collapse-<?= $child['id'] ?>" style="display: none">
            ...
            <div class="widget widget-categories mb-4 pb-4 border-bottom">
              ...
              <div class="accordion mt-n1" id="shop-categories">
                ...
                <div class="card">
                  ...
                  <div class="card-body ml-2" id="collapsem-<?= $category['id'] ?>" style="display: none">
                    @foreach($category->childes as $child)
                    <div class="card-header p-1">
                      <label class="for-hover-lable" style="cursor: pointer" onclick="location.href='<?= route('products', ['id' => $child['id'], 'data_from' => 'category', 'page' => 1]) ?>'"><?= $child['name'] ?></label>
                      <strong class="pull-right for-brand-hover" style="cursor: pointer" onclick="$('#collapsem-<?= $child['id'] ?>').toggle(300)">
                        <?= $child->childes->count() > 0 ? '+' : '' ?>
                      </strong>
                    </div>
                    <div class="card-body ml-2" id="collapsem-<?= $child['id'] ?>" style="display: none">
                      @foreach($child->childes as $ch)
                      <div class="card-header p-1">
                        <label class="for-hover-lable" style="cursor: pointer" onclick="location.href='<?= route('products', ['id' => $ch['id'], 'data_from' => 'category', 'page' => 1]) ?>'"><?= $ch['name'] ?></label>
                      </div>
