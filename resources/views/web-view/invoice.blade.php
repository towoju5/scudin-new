...
<body>
    ...
    $e_commerce_logo = BusinessSetting::where(['type'=>'company_web_logo'])->first()->value;
    @endphp
    ...
    <div class="first" style="display: block; height:auto !important;">
        <table>
            <tr>
                <div class="row">
                    <div style="padding:15px;" header>
                        <div style="float: left;">
                            <img height="70" width="200" src="{{ asset($e_commerce_logo) }}" alt="{{$e_commerce_logo}}"> 
                            <!-- <p style="margin-top: 0px; font-weight:900; font-size:xxx-large">{{$company_name}}</p> -->
                            <p style="margin-top: 5px; margin-bottom: 0px;">Phone Number:{{$company_phone}}</p>
                            <p style="margin-top: 5px; margin-bottom: 0px;">Email:{{$company_email}}</p>
                        </div>
                        <div style="float: right;padding: 0 15px 15px 15px">
                            <h1 style="color: #030303; margin-bottom: 0px;">INVOICE</h1>
                            <h3 style="color: #030303; margin-top: 4px; margin-bottom:2px;">Invoice ID:
                                <strong style="color: <?= $web_config['primary_color'] ?>;font-size:20px;"> #{{ $order->id }}</strong>
                            </h3>
                        </div>
                    </div>
                </div>
                    ...
                    <tr>
                        <td style="width: 41%">
                            <div style="float: left">
                                ...
                                <p style=" margin-top: 6px; margin-bottom:0px;">{{$order->shipping ? $order->shipping['address'] : ""}}</p>
        ...
        <table style="width: 46%;margin-left:auto;  " class="text-right sm-padding  strong">
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-right"">{{\App\CPU\BackEndHelper::usd_to_currency($sub_total)}} {{\App\CPU\BackEndHelper::currency_code()}}</td>

                </tr>
                <tr>
                    <th class=" gry-color text-left strong bold">TAX</th>
                    <td class="text-right"">{{\App\CPU\BackEndHelper::usd_to_currency($total_tax)}} {{\App\CPU\BackEndHelper::currency_code()}}</td>
                </tr>
                <tr>
                    <th class=" gry-color text-left strong bold">Shipping</th>
                    <td class="text-right"">{{\App\CPU\BackEndHelper::usd_to_currency($total_shipping_cost)}} {{\App\CPU\BackEndHelper::currency_code()}}</td>
                </tr>
                <tr>
                    <th class=" gry-color text-left strong bold">Coupon Discount</th>
                    <td class="text-right"">- {{\App\CPU\BackEndHelper::usd_to_currency($order->discount)}} {{\App\CPU\BackEndHelper::currency_code()}}</td>
                </tr>
                <tr class=" border-bottom">
                    <th class="gry-color text-left strong bold">Discount on Product</th>
                    <td class="text-right"">- {{\App\CPU\BackEndHelper::usd_to_currency($total_discount_on_product)}} {{\App\CPU\BackEndHelper::currency_code()}}</td>
                </tr>
                <tr>
                    <th class=" gry-color text-left strong bold">Total</th>
                    <td class="bold text-right">{{\App\CPU\BackEndHelper::usd_to_currency($order->order_amount)}} {{\App\CPU\BackEndHelper::currency_code()}}</td>
                </tr>
            </tbody>
        </table>
    </div>
