...
class CartManager
{
    ...
    public static function shipping_cost($cart)
    {
        foreach ($cart as $item) {
            $total = 0;
            if (!empty($cart)) {
                foreach ($cart as $item) {
                    ...
                    $shipping_id = $item['shipping_method_id'];
                    $_shipFee = get_shipping_price_by_id($shipping_id, $product['weight'], $product['height'], $product['width'], $product['length'], $product['unit']);
                    $product_subtotal = $_shipFee * $item['quantity'];
                    $total += $product_subtotal;
                }
            }
            return $total;
