...
class ProductManager
{
    ...
    public static function get_shipping_methods($product)
    {
        if ($product['added_by'] == 'seller') {
            $methods = ShippingMethod::where(['creator_id' => $product['user_id']])->where('id', '!=', 1)->where(['status' => 1])->get();
            if ($methods->count() == 0) {
                $methods = ShippingMethod::where(['creator_type' => 'admin'])->where(['status' => 1])->get();
            }
        } else {
            $methods = ShippingMethod::where(['creator_type' => 'admin'])->where(['status' => 1])->where('id', '!=', 1)->get();
        }

        foreach ($methods as $method) {
            if($method['method'] != "default"){
                $method['cost'] = $methods['cost'] ?? 0; //get_shipping_price_by_id($method['id'], $product['weight'], $product['height'], $product['width'], $product['length'], $product['unit']);
            //    $methods['cost'] ?? 0;
            }
        }
    ...
    public static function check_ethos_status($seller_id, $limit = 10, $offset = 1)
    {
        $seller_ethos_status = Seller::find($seller_id);
        $user_ethos_status = User::find(auth('customer')->id());
        if (!empty($seller_ethos_status) && $seller_ethos_status->plan_id == 1) {
            return true;
        } else if (!empty($user_ethos_status) && ($user_ethos_status->plan_id == 1)) {
            return true;
        } else {
            return false;
        }
