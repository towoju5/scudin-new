...
class OrderController extends Controller
{
    function list(Request $request, $status) {
        ...
        if ($status != 'all') {
            $orders = OrderDetail::with('order.customer', 'order')->where(['seller_id' => auth('seller')->id(), 'delivery_status' => $status])->latest()->paginate(15);
        } else {
            $orders = OrderDetail::with('order.customer', 'order')->where(['seller_id' => auth('seller')->id()])->latest()->paginate(15);

        }

        if($request->has('order') && !empty($request->order)){
            $stat = $request->order;
            $orders = OrderDetail::where('order_id', $stat)->latest()->paginate(15);
        }
    ...
    public function productStatus(Request $request)
    {
        if ($request->ajax()) {
            ...
            $_orderInfo = Product::find($order->product_id);

            // Grab category info via charge_cat
            $p_cat = Category::find($_orderInfo->charge_cat);
            if($p_cat->commision_type == 'combined_fee'){
                $commission = (((($order->price / 100) * $p_cat->percentage) * $order->qty) - $p_cat->flat);
            } elseif($p_cat->commision_type == 'flat_fee') {
                $commission = (($order->price * $order->qty) - ($p_cat->flat * $order->qty));
            } elseif($p_cat->commision_type == 'percentage') {
                $commission = (($order->price / 100) * $p_cat->percentage) * $order->qty;
            }
