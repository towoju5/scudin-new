...
class OrderController extends Controller
{
    function list(Request $request, $status) {
        ...
        if($request->has('order') && !empty($request->order)){
            $stat = $request->order;
            $orders = Order::where('id', $stat)->with(['customer'])->latest()->paginate(15);
        }
    ...
    public function generate_invoice($id)
    {
        ...
        // $data["address"] = ShippingAddress::find($order->shipping_address);
        $data["email"] = $order->customer["email"];
        $data["client_name"] = $order->customer["f_name"] . ' ' . $order->customer["l_name"];
        $data["order"] = $order;
        $rrr = $order->details->toArray();
        $data['p_name'] = json_decode($rrr[0]['product_details'], true);
