...
class OrderController extends Controller
{
    function list($status) {
        ...
        if ($status != 'all') {
            $orders = OrderDetail::with('order.customer', 'order')->where(['seller_id' => auth('staff')->user()->seller_code, 'delivery_status' => $status])->latest()->paginate(15);

            // $orders = OrderDetail::with('order.customer', 'order')->where(['seller_id' => auth('staff')->user()->seller_code])->orWhere([['delivery_status' => $status]])->latest()->paginate(25);

        } else {
            $orders = OrderDetail::with('order.customer', 'order')->where(['seller_id' => auth('staff')->user()->seller_code])->latest()->paginate(15);

        }

        // $orders = OrderDetail::with('order.customer', 'order')->where(['seller_id' => auth('staff')->user()->seller_code])->latest()->paginate(25);
        return view('staff-views.order.list', compact('orders'));
    }
    ...
    public function productStatus(Request $request)
    {
        if ($request->ajax()) {
            ...
            $orders->order_status = $request->delivery_status;
            $orders->save();
    ...
    public function details($id)
    {
        ...
        $order = Order::with(['details' => function ($query) use ($sellerId) {
            ...
            ->where('id', $id)->first();
        return view('staff-views.order.order-details', compact('order'));
    }
