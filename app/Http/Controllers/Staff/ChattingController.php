...
class ChattingController extends Controller
{
    ...
    {
        $shop_id = Shop::where('seller_id', auth('staff')->user()->seller_code)->first()->id;
        ...
        if (isset($last_chat)) {
            ...
            $chattings = Chatting::join('tbl_users', 'tbl_users.id', '=', 'chattings.user_id')
                ->select('chattings.*', 'tbl_users.f_name', 'tbl_users.l_name', 'tbl_users.image')
                ->where('chattings.shop_id', $shop_id)
                ->where('user_id', $last_chat->user_id)
                ->get();

            $chattings_user = Chatting::join('tbl_users', 'tbl_users.id', '=', 'chattings.user_id')
                ->select('chattings.*', 'tbl_users.f_name', 'tbl_users.l_name', 'tbl_users.image')
                ->where('chattings.shop_id', $shop_id)
    ...
    {
        $shop_id = Shop::where('seller_id', auth('staff')->user()->seller_code)->first()->id;
        $last_chat = Chatting::where('seller_id', auth('staff')->user()->seller_code)
        ...
        $sellers = Chatting::join('tbl_users', 'tbl_users.id', '=', 'chattings.user_id')
            ->select('chattings.*', 'tbl_users.f_name', 'tbl_users.l_name', 'tbl_users.image')
            ->where('chattings.shop_id', $shop_id)
